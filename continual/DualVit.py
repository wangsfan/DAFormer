...
class Block(nn.Module):
    ...
    def forward(self, x, key=None):
        xx = self.norm1(x)
        if isinstance(self.attn, Dual_Attention_bn):
            xx, attn, v = self.attn(xx, key)
        else:
...
class Dual_Attention_bn(nn.Module):
    def __init__(self, dim, num_heads=8, attn_drop=0., proj_drop=0., qk_scale=None, qkv_bias=None,fc=nn.Linear):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = qk_scale or head_dim ** -0.5

        self.to_q = fc(dim, dim, bias=qkv_bias)
        self.to_v = fc(dim, dim, bias=qkv_bias)
        self.to_k = fc(dim, dim, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
    ...
    def forward(self, x, key):
        ...
        k1 = self.to_k(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = self.to_v(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

        q = q * self.scale

        atten_inter = (q @ k1.transpose(-2, -1)).softmax(dim=-1)
        atten_inter = self.attn_drop(atten_inter)

        atten_intra = key(q.permute(0, 3, 1, 2)).permute(0, 2, 3, 1).softmax(dim=-1)

        out1 = (atten_inter @ v).transpose(1, 2).reshape(B, N, C)
        out2 = (atten_intra @ v).transpose(1, 2).reshape(B, N, C)

        out = out1+out2

        return out, atten_inter, atten_intra
...
class DualVit(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_chans=3, num_classes=1000, embed_dim=384, depth=6,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer='layer', patchs=64,
        ...
        self.key = nn.Conv2d(embed_dim//num_heads, patchs, 1)
