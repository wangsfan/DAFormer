...
def get_args_parser():
    parser = argparse.ArgumentParser('Dual training and evaluation script', add_help=False)
    ...
    parser.add_argument('--output-dir', default='',
                        help='Dont use that')
    parser.add_argument('--output-basedir', default='',
    ...
    parser.add_argument('--options', default=["options/data/cifar100_10-10.yaml", "options/data/cifar100_order1.yaml", "options/model/cifar_dual.yaml"], nargs='*')
    # parser.add_argument('--options', default=[], nargs='*')

    # hyperDDE related
    parser.add_argument('--dual', action='store_true', default=True,
                        help='Enable super hyper god mode.')
    ...
    parser.add_argument('--head-div', default=0., type=float,
                        help='Use a divergent head to predict among new classes + 1 using cross token')
    ...
    parser.add_argument('--replay-memory', default=2000, type=int,
    ...
    parser.add_argument('--start-epoch', default=0, type=int, help='   from checkpoint')
...
def main(args):
    print(args)
    print("is a distributed training?----->"+str(args.distributed))
    # print("number of GPUs: ", args.gpu)
    ...
    for task_id, dataset_train in enumerate(scenario_train):
        ...
        # Ensembling
        if args.dual:
            model_without_ddp = factory.update_dual(model_without_ddp, task_id, args)
        ...
        if task_id > 0 and not args.dual:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('dual training and evaluation script', parents=[get_args_parser()])
